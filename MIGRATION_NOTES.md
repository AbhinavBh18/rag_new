# Refactor Notes — LlamaIndex → LangChain, Chroma → Pinecone, Hybrid Retrieval, Memory, Evaluation
...
| `src/indexing.py` | `src/indexing.py` | Rewritten (Pinecone) |
...
python -m src.indexing        # Phase 3 — builds Pinecone index + chunk store
...
| `VectorStoreIndex` + `StorageContext` | `PineconeVectorStore` | One object |
...
Done, not skipped. Setup turned out to be modest: a free serverless index, an API key, and `langchain-pinecone`.

Genuine differences that forced code changes:

1. **Dimension is immutable.** `create_index(dimension=384, metric="cosine")` is set once. Change the embedding model and you must delete and recreate the index. `EMBEDDING_DIMENSION` in config is now load-bearing rather than decorative.
2. **Index creation is async.** `ensure_index()` polls `describe_index()` until ready.
3. **Eventual consistency.** `describe_index_stats()` lags an upsert by a few seconds, so the Phase 3 verification sleeps before reading the count. Do not treat a momentary `0` as failure.
4. **No bulk export.** Chroma doubled as a local docstore; Pinecone does not. BM25 needs the full corpus in RAM, so `parsing.py` now writes `data/processed/chunks.jsonl` and that file is the source of truth for sparse retrieval. This is the single most important structural consequence of the migration.
5. **Batched upserts.** Pinecone caps requests at ~2 MB / 1000 vectors; `UPSERT_BATCH_SIZE = 100`.
6. **Deterministic IDs** (`lora.pdf::p004::c02`) make re-indexing idempotent instead of duplicating vectors.
7. **Package rename.** `pinecone-client` is now `pinecone`. Having both installed breaks imports — uninstall the old one.

### Blocker: Pinecone needs network + a key

If you need to run fully offline (or your corporate network blocks it), swap the vector store for local FAISS. It is a two-line change in `indexing.py` because everything downstream only depends on the LangChain `VectorStore` interface:

```python
from langchain_community.vectorstores import FAISS
vector_store = FAISS.from_documents(chunks, get_embedding_model())
vector_store.save_local(str(PROJECT_ROOT / "data" / "faiss"))
```

`retrieval.py`, `pipeline.py`, `app.py`, and `evaluate.py` need no edits.

---

## 3. Hybrid retrieval

`src/retrieval.py`. Dense (Pinecone) and sparse (BM25) each return 10 candidates; they are fused by **weighted Reciprocal Rank Fusion** and cut to the top 5.
...
Failure isolation: if either retriever raises, the fusion logs it and continues with the other, so a Pinecone outage degrades to keyword search instead of a 500.

### Scale caveat
`BM25Retriever` holds the whole corpus in memory and rebuilds statistics at startup. Fine for 8 papers / a few thousand chunks. At millions of chunks you would move to Pinecone's native sparse-dense hybrid vectors or an OpenSearch BM25 backend. Worth saying out loud in an interview — it shows you know where the design stops working.
